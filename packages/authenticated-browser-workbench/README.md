...
登录态增强检索是AI默认网页搜索的补充层。公开搜索足够时不启动浏览器；信息不足、需要真实体验或关键内容必须登录时，再调用BrowserSkill进入使用者已经登录的网站补充检索。
...
当前为 `v0.1.0-candidate.2` 公开候选版。候选版增加了公开搜索充分性判断、登录态渠道自动回退和证据分层；尚未覆盖更多操作系统、浏览器版本、休眠唤醒和长任务场景。测试范围见 `TEST_MATRIX.md`，风险见 `KNOWN_LIMITATIONS.md`。
