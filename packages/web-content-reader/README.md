...
Web Content Reader 是一套批量读取微信公众号文章和普通公开网页的正式版 Workflow。它包含两个 Skill：
...
当前 Workflow 版本为`v0.2.0`，内含`Weixin Article Reader v0.1.0`。默认隐身提取单次设置45秒硬超时，避免不可达页面无限阻塞整批任务。测试范围与失败情况见`TEST_MATRIX.md`和`KNOWN_LIMITATIONS.md`。本包不包含API Key、Cookie、浏览器Profile、代理服务或任何登录信息。
