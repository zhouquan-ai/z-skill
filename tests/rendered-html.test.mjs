...
test("server-renders the z-skill brand homepage", async () => {
  ...
  assert.match(html, /每项工具都说明三件事/);
  assert.doesNotMatch(html, /class="eyebrow">个人 AI 工具发布站/);
  ...
  assert.doesNotMatch(html, /PERSONAL AI TOOLS|RELEASES|PRINCIPLES/);
...
test("server-renders the 简历库与简历管理 public candidate", async () => {
  const response = await render("/tools/resume-library-management");
  assert.equal(response.status, 200);

  const html = await response.text();
  assert.match(html, /<title>简历库与简历管理｜z-skill<\/title>/i);
  assert.match(html, /Workflow/);
  assert.match(html, /公开候选/);
  assert.match(html, /v0\.1\.0-candidate\.1/);
  assert.match(html, /来源、职业事实、经历模块、岗位画像和输出/);
  assert.match(html, /resume-library-management-v0\.1\.0-candidate\.1\.zip/);
